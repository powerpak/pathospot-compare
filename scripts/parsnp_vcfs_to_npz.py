...
def read_vcfs(parsnp_vcfs, in_paths=None, quiet=False):
    ...
    for i, vcf_file in enumerate(parsnp_vcfs):
        ...
        if in_paths is not None:
            ...
            ref_bed = re.sub(r'\.fa(sta)?$', '.bed', ref_fasta)
            if (isfile(ref_fasta) and access(ref_fasta, R_OK) and 
                    isfile(ref_bed) and access(ref_bed, R_OK)):
                vcf_allele_info = enhance_allele_info(vcf_allele_info, ref_fasta, ref_bed, **opts)
                vcf_data['ref_chrom_sizes_%d' % i] = fasta_chrom_sizes(ref_fasta)
            elif not quiet:
                sys.stderr.write("WARN: Couldn't find .fasta + .bed annotations for %s\n" % ref_seq)
